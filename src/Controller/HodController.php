...
class HodController extends BaseController {
    ...
    private function sendCredentialsMessage($db, $userId, $firstName, $lastName, $email, $cnic, $password, $portalType) {
        ...
        $lowerPortal = strtolower($portalType);
        if (strpos($lowerPortal, 'coordinator') !== false) {
            $profileRoute = '/coordinator/profile';
        } elseif (strpos($lowerPortal, 'committee') !== false) {
            $profileRoute = '/committee/profile';
        } elseif (strpos($lowerPortal, 'hod') !== false) {
            $profileRoute = '/hod/profile';
        } elseif (strpos($lowerPortal, 'student') !== false) {
            $profileRoute = '/student/profile';
        }

        $stmtN = $db->prepare("INSERT INTO notifications (user_id, title, message, redirect_url) VALUES (?, ?, ?, ?)");
        $stmtN->execute([$userId, $welcomeTitle, $welcomeMsg, $profileRoute]);
    }
